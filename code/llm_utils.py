...
#             # Capture the raw logprob output
#             if response.candidates and hasattr(response.candidates[0], "logprobs_result"):
#                 lp_obj = response.candidates[0].logprobs_result
#                 logprobs_text = str(lp_obj)
#                 print(f"[DEBUG] Gemini logprobs_result captured ({len(logprobs_text)} chars)")
#             else:
#                 print("[DEBUG] Gemini response had no logprobs_result field")

#             # Always attempt to write if we have any text
#             if log_dir:
#                 os.makedirs(log_dir, exist_ok=True)
#                 ts = time.strftime("%Y%m%d_%H%M%S")
#                 out_path = os.path.join(log_dir, f"logprobs_{ts}.txt")
#                 with open(out_path, "w", encoding="utf-8") as f:
#                     f.write(logprobs_text or "No logprob data captured.")
#                 print(f"[DEBUG] Logprob text written to {out_path}")
...
#                 print(f"[DEBUG] OpenAI logprobs written to {log_dir}")
#         except Exception as e:
